...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(self, x):
        weight = paddle.ones(shape=[x.shape[-1]], dtype="float32")
        weight.stop_gradient = False
        bias = paddle.ones(shape=[x.shape[-1]], dtype="float32")
        bias.stop_gradient = False
        out = paddle.nn.functional.layer_norm(
            x, self.reduce, weight, bias, self.variance_epsilon
        )
        return out
...
class TestRMSNormSubGraphDD(unittest.TestCase):
    ...
    def train_one_window(self, i, j):
        ...
        with open('./layernorm_dd_comp.csv', 'a') as fp:
            print(str(calculate_average(sorted_times[500:-500])), file=fp)
...
class TestRMSNormSubGraphDS(unittest.TestCase):
    ...
    def train_one_window(self, i, j):
        ...
        with open('./layernorm_ds_comp.csv', 'a') as fp:
            print(str(calculate_average(sorted_times[500:-500])), file=fp)
...
class TestRMSNormSubGraphSD(unittest.TestCase):
    ...
    def train_one_window(self, i, j):
        ...
        with open('./layernorm_sd_comp.csv', 'a') as fp:
            print(str(calculate_average(sorted_times[500:-500])), file=fp)
